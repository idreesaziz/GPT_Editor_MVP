...
from .plugins.moviepy_plugin import MoviePyPlugin
...
PLUGIN_REGISTRY: Dict[str, ToolPlugin] = {
    p.name: p for p in [FFmpegPlugin(), MetadataExtractorPlugin(), MoviePyPlugin()]
}
...
def process_complex_request(session_path: str, prompt: str, initial_proxy_name: str, run_logger: logging.Logger) -> Dict[str, Any]:
    ...
    """
    run_logger.info("=" * 20 + " ORCHESTRATOR " + "=" * 20)
    ...
    with Timer(run_logger, "Total Orchestration Process"):
        try:
            ...
            with Timer(run_logger, "Intermediate File Cleanup", level=logging.DEBUG):
            ...
            run_logger.info(f"ORCHESTRATOR: Final output: {final_output_filename}")
