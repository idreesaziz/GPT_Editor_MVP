...
# --- Constants for iterative generation (The Specialist's internal process) ---
MAX_RETRIES = 3
CANDIDATES_FIRST = 1
CANDIDATES_RETRY = 3
...
def generate_validated_script(task: str, plugin: ToolPlugin, context: Dict[str, Any], inputs: Dict, outputs: Dict) -> str:
    """
    Generates and validates a SINGLE script for a given task using a specific plugin.
    This function will retry internally until a valid script is produced or retries are exhausted.
    """
    ...
    for attempt in range(MAX_RETRIES):
        ...
        # *** THE FIX IS HERE ***
        # We now correctly extract the 'completed_steps_log' from the context
        # dictionary and pass it explicitly to the template.
        completed_steps_log_data = context.get("completed_steps_log", [])
        ...
        user_content = USER_CONTENT_TEMPLATE.format(
            ...
            outputs=str(outputs),
            context=str(context),
            completed_steps_log=str(completed_steps_log_data)
        )
        ...
        logger.debug(f"--- Script Gen Prompt (Attempt {attempt+1}) ---\n{full_prompt}\n---")
        ...
        for i, candidate in enumerate(response.candidates):
            ...
            # Delegate validation to the plugin
            is_valid, error_msg = plugin.validate_script(script_content)
